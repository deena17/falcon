...
class IonAuthModel 
{
	...
	public function activate($id, string $code=''): bool
	{
		...
		if (!$code || ($user && $user->id == $id))
		{
			...
			$this->db->table($this->tables['auth_users'])->update($data, ['id' => $id]);
	...
	public function clearForgottenPasswordCode(string $identity): bool
	{
		...
		return $this->db->table($this->tables['auth_users'])->where($this->identityColumn, $identity)->update($data);
	}
	...
	public function changePassword(string $identity, string $old, string $new): bool
	{
		...
		$builder = $this->db->table($this->tables['auth_users']);
		$query   = $builder
					   ->select('id, password')
	...
	public function usernameCheck(string $username): bool
	{
		...
		return $this->db->table($this->tables['auth_users'])
	...
	public function forgottenPassword(string $identity)
	{
		...
		$this->triggerEvents('extra_where');
		$this->db->table($this->tables['auth_users'])->update($update, [$this->identityColumn => $identity]);
	 ...
	 * @author Mathew
	 */
	public function register(string $identity, string $password, string $email, array $additionalData=[], array $groups=[], array $departments=[], array $permissions=[])
	{
		...
		$query = $this->db->table($this->tables['auth_groups'])->where(['name' => $this->config->defaultGroup], 1)->get()->getRow();
		...
		$userData = array_merge($this->filterData($this->tables['auth_users'], $additionalData), $data);

		$this->triggerEvents('extra_set');

		$this->db->table($this->tables['auth_users'])->insert($userData);

		$id = $this->db->insertId($this->tables['auth_users'] . '_id_seq');
		...
		$userPermission = new \App\Models\UserPermissionModel();
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		$this->triggerEvents('extra_where');
		$query = $this->db->table($this->tables['auth_users'])
		...
		if (isset($user))
		{
			if ($this->verifyPassword($password, $user->password, $identity))
			{
				...
				// $permissions = $this->db->table('auth_users_permissions')->getWhere('user_id', $user->id);
	...
	public function getAttemptsNum(string $identity, $ipAddress=null): int
	{
		if ($this->config->trackLoginAttempts)
		{
			$builder = $this->db->table($this->tables['auth_login_attempts']);
	...
	public function users($groups=null): self
	{
		...
		else
		{
			...
			$builder->select([
				...
				$this->tables['auth_users'] . '.id as id',
				$this->tables['auth_users'] . '.id as user_id',
			]);
		}
		...
		if (isset($groups))
		{
			...
			if (! empty($groups))
			{
				$builder->distinct();
				$builder->join(
					$this->tables['auth_users_groups'],
					$this->tables['auth_users_groups'] . '.' . $this->join['auth_users'] . '=' . $this->tables['auth_users'] . '.id',
					'inner'
				);
			}
			...
			if (! empty($groupNames))
			{
				$builder->join($this->tables['auth_groups'], $this->tables['auth_users_groups'] . '.' . $this->join['auth_groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner');
				$builder->whereIn($this->tables['auth_groups'] . '.name', $groupNames);
			}
			if (! empty($groupIds))
			{
				$builder->{$orWhereIn}($this->tables['auth_users_groups'] . '.' . $this->join['auth_groups'], $groupIds);
			}
		}
	...
	public function user(int $id=0): self
	{
		...
		$this->limit(1);
		$this->orderBy($this->tables['auth_users'] . '.id', 'desc');
		$this->where($this->tables['auth_users'] . '.id', $id);
	...
	public function getUsersGroups(int $id=0)
	{
		...
		$builder = $this->db->table($this->tables['auth_users_groups']);
		return $builder->select($this->tables['auth_users_groups'] . '.' . $this->join['auth_groups'] . ' as id, ' . $this->tables['auth_groups'] . '.name, ' . $this->tables['auth_groups'] . '.description')
					   ->where($this->tables['auth_users_groups'] . '.' . $this->join['auth_users'], $id)
					   ->join($this->tables['auth_groups'], $this->tables['auth_users_groups'] . '.' . $this->join['auth_groups'] . '=' . $this->tables['auth_groups'] . '.id')
					   ->get();
	}
	...
	public function addToGroup($groupIds, int $userId=0): int
	{
		...
		foreach ($groupIds as $groupId)
		{
			...
			if ($this->db->table($this->tables['auth_users_groups'])->insert([
																	$this->join['auth_groups'] => (float)$groupId,
																	$this->join['auth_users']  => (float)$userId  ]))
			{
	...
	{
		$this->triggerEvents('auth_groups');

		$builder = $this->db->table($this->tables['auth_groups']);
	...
	public function group(int $id=0)
	{
		...
		if ($id)
		{
			$this->where($this->tables['auth_groups'] . '.id', $id);
		}
	...
	public function update(int $id, array $data): bool
	{
		...
		$this->triggerEvents('extra_where');
		$this->db->table($this->tables['auth_users'])->update($data, ['id' => $user->id]);
	...
	public function deleteUser(int $id): bool
	{
		...
		$this->db->table($this->tables['auth_users'])->delete(['id' => $id]);
	...
	public function updateLastLogin(int $id): bool
	{
		...
		$this->db->table($this->tables['auth_users'])->update(['last_login' => time()], ['id' => $id]);
	...
	public function rememberUser(string $identity): bool
	{
		...
		{
			$this->db->table($this->tables['auth_users'])->update(['remember_selector' => $token->selector,
	...
	public function createGroup(string $groupName='', string $groupDescription='', array $additionalData=[])
	{
		...
		$existingGroup = $this->db->table($this->tables['auth_groups'])->where(['name' => $groupName])->countAllResults();
		if ($existingGroup !== 0)
		{
		...
		{
			$data = array_merge($this->filterData($this->tables['auth_groups'], $additionalData), $data);
		}

		$this->triggerEvents('extra_group_set');

		// insert the new group
		$this->db->table($this->tables['auth_groups'])->insert($data);
		$groupId = $this->db->insertId($this->tables['auth_groups'] . '_id_seq');
	...
	public function updateGroup(int $groupId, string $groupName='', array $additionalData=[]): bool
	{
		...
		if (! empty($groupName))
		{
			...
			// bail if the group name already exists
			$existingGroup = $this->db->table($this->tables['auth_groups'])->getWhere(['name' => $groupName])->getRow();
		...
		$group = $this->db->table($this->tables['auth_groups'])->getWhere(['id' => $groupId])->getRow();
		...
		$this->db->table($this->tables['auth_groups'])->update($data, ['id' => $groupId]);
	...
	public function deleteGroup(int $groupId): bool
	{
		...
		$this->db->table($this->tables['users_groups'])->delete([$this->join['auth_groups'] => $groupId]);
		// remove the group itself
		$this->db->table($this->tables['auth_groups'])->delete(['id' => $groupId]);
	...
	protected function setPasswordDb(string $identity, string $password): bool
	{
		...
		$this->db->table($this->tables['auth_users'])->update($data, [$this->identityColumn => $identity]);
