...
class IonAuth
{
	 ...
	 * @author Mathew
	 */
	public function register(string $identity, string $password, string $email, array $additionalData = [], array $groupIds = [], array $departments=[], array $permissions=[])
	{
		$this->ionAuthModel->triggerEvents('pre_account_creation');

		$emailActivation = $this->config->emailActivation;

		$id = $this->ionAuthModel->register($identity, $password, $email, $additionalData, $groupIds, $departments, $permissions);
	...
	public function checkPermission($permission){
		...
		$builder = $db->table('auth_permissions');
		$builder->select('code');
		$builder->join('auth_users_permissions', 'auth_permissions.id = auth_users_permissions.permission_id');
		$builder->where('auth_users_permissions.user_id', $userId);
		$builder->where('auth_permissions.code', $permission);
