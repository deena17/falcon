...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item">Invoice Status</li>
            ...
            <div class="card-body">
                <form action="" id="myForm">
                    <div class="form-group">
                        ...
                        <input type="text" id="status" class="form-control" name="code">
...
<script>
    $("document").ready(function(){
        function loadInvoiceStatus(){
            $.get("<?= site_url(); ?>api/invoice-status", function(response, status){
                var listItems = '';
        ...
        $(".list-group").on('click', '.btn-primary', function(){
            var {id} = $(this).data(id);
            resetForm();
            $.get(`<?= site_url(); ?>api/invoice-status/${id}`, function(response, status){
            ...
            $(".form-group").on('click', '.btn-info', function(){
                ...
                $.ajax({
                url: `<?= site_url(); ?>api/invoice-status/${id}`,
                type: 'PUT',
                dataType: 'json',
                data:{id:id, status:status },
                success: function(result) {
                    alert("Invoice status details updated successfully");
                    resetForm();
                    loadInvoiceStatus();
                }
            });
        ...
        $(".form-group").on('click', '.btn-success', function(){
            const [status] = validateForm();
            $.ajax({
                url: `<?= site_url(); ?>api/invoice-status`,
                type: 'POST',
                dataType: 'json',
                data:{status:status},
                success: function(result) {
                    alert("Invoice status details added successfully");
                    resetForm();
                    loadInvoiceStatus();
                }
            });
