...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item">Call Type</li>
...
<div class="row">
    ...
    <div class="col-md-5">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h5 class="card-title">Create/Edit Call Type</h5>
            </div>
            <div class="card-body">
                <form action="" id="myForm" data-id="">
                    <div class="form-group">
                        <label for="department-code">Call Type</label>
...
<script>
    $("document").ready(function(){
        function loadCallType(){
            $.get("<?= site_url(); ?>api/call-type", function(response, status){
                var listItems = '';
        ...
        $(".list-group").on('click', '.btn-primary', function(){
            var {id} = $(this).data(id);
            resetForm();
            $.get(`<?= site_url(); ?>api/call-type/${id}`, function(response, status){
            ...
            $(".form-group").on('click', '.btn-info', function(){
                ...
                $.ajax({
                url: `<?= site_url(); ?>api/call-type/${id}`,
                type: 'PUT',
                dataType: 'json',
                data:{id:id, related:type },
                success: function(result) {
                    alert("Call type details updated successfully");
                    resetForm();
                    loadCallType();
                }
            });
        ...
        $(".list-group").on('click', '.btn-danger', function(){
            var {id} = $(this).data(id);
            if(confirm("Are you sure want to delete the call type?")){
                $.ajax({
                    url: `<?= site_url(); ?>api/call-type/${id}`,
                    type: 'DELETE',
                    success: function(result) {
        ...
        $(".form-group").on('click', '.btn-success', function(){
            const [type] = validateForm();
            $.ajax({
                url: `<?= site_url(); ?>api/call-type`,
                type: 'POST',
                dataType: 'json',
                data:{ type:type },
                success: function(result) {
                    alert("Call type details added successfully");
                    resetForm();
                    loadCallType();
                }
            });
